    ...
    <main className="min-h-screen bg-[#f7f5f3] text-[#37322f]">
      ...
      <section className="mx-auto max-w-[1060px] px-4 pb-16 pt-24">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-4">
          {stats.map((item) => (
            <div key={item.label} className="rounded-lg border border-[#e0dedb] bg-white p-5">
              <p className="text-xs uppercase tracking-wide text-[#7b736d]">{item.label}</p>
              <p className="mt-3 text-3xl font-semibold">{item.value}</p>
            </div>
          ))}
        </div>

        <div className="mt-6 rounded-lg border border-[#e0dedb] bg-white p-6">
          <h2 className="text-xl font-semibold">Upcoming Volunteer Events</h2>
          <div className="mt-4 space-y-3">
            {upcoming.map((event) => (
              <div key={event.title} className="rounded-md border border-[#e0dedb] p-4">
                <p className="font-semibold">{event.title}</p>
                <p className="text-sm text-[#605a57]">{event.organization}</p>
                <p className="mt-1 text-sm text-[#605a57]">{event.time}</p>
              </div>
            ))}
          </div>
        </div>
